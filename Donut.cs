...
namespace gdnoob_Program
{
    internal static class Program
    {
            ...
            while (true)
            {
                memset(b, ' ', 1760); memset(z, 0.0f, 7040);
                for (j = 0; 6.28 > j; j += 0.07)
                    for (i = 0; 6.28 > i; i += 0.02)
                    {
                        ...
                        double l = Math.Cos(i);
                        double m = Math.Cos(B); double n = Math.Sin(B);
                        double t = (c * h * g) - (f * e); int x = (int)(40 + (30 * D * ((l * h * m) - (t * n))));
